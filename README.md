...
6. Wire the behavioral instruction so future agent runs query specs via
   the CLI instead of grepping. Skip if the user already has the snippet
   at a user-global location (~/.claude/CLAUDE.md, ~/.cursor/rules/,
   etc.). Otherwise detect the repo-local agent config file and append
...
Without this step, the agent has the tool installed but doesn't know to use it — it will keep defaulting to `ls docs/specs/` + grep + Read on every spec question.

**Skip this step if the user already has the snippet at a user-global location** (e.g. `~/.claude/CLAUDE.md`, `~/.cursor/rules/`, or equivalent) that applies across all their repos. The behavioral instruction's job is "agent knows the CLI exists" — repo-local is the safe default when nothing else exists, but user-global accomplishes the same thing without per-repo duplication.

Otherwise, pick the file that matches the user's agent and append (or create) it:
