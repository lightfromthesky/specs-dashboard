...
Recognised values: Shipped <date>, In progress, Draft, Planned,
Closed <date>, Abandoned <date>, Superseded by <path>.

Legacy values (Completed, Live, Implemented, Pending) are tolerated
by default and mapped to the canonical buckets. Pass
...
def classify_status(raw: str, legacy_aliases: bool) -> tuple[str, str | None]:
    ...
    Bucket is one of: Shipped, In progress, Draft, Planned, Closed,
    Abandoned, Superseded, Unknown. Date is the first YYYY-MM-DD found
    in the value, or None.
    """
    lower = raw.lower()
...
def cmd_serve(args: argparse.Namespace) -> None:
    """Run a local HTTP server that reads specs at request time.

    The HTML is the same template `build` produces, but with an empty
    embedded data block — JS fetches /api/specs on load and applies
    the live snapshot. Page heartbeats /api/ping every 5 min while
    visible; server auto-shuts-down after `idle_timeout` seconds with
    no requests, so leaving the process running is the default mode.
    """
    input_dir = _resolve_input(args.input)
    DashboardHandler.input_dir = input_dir
    DashboardHandler.excludes = set(args.exclude)
    DashboardHandler.legacy_aliases = args.legacy_aliases
    DashboardHandler.title = args.title
    ...
    print(f"Serving specs from {input_dir} at {url}")
    if args.idle_timeout > 0:
        print(
            f"  auto-shutdown after {args.idle_timeout}s idle "
            f"(--no-idle-timeout to disable)"
        )
    print("  Ctrl-C to stop.")

    def shutdown_handler(signum=None, frame=None) -> None:
        # `shutdown` blocks until serve_forever returns, so run it off-thread.
    ...
    if not args.no_browser:
        # Tiny delay so the server is accepting connections before browser hits it.
...
def _add_common_args(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        ...
        dest="legacy_aliases",
        action="store_false",
        help="Treat Completed/Live/Implemented/Pending as Unknown.",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    p_serve = sub.add_parser(
        "serve",
        help="Run a local HTTP server that reads specs at request time (no regen step)",
    )
    ...
    p_serve.add_argument(
        ...
        type=int,
        default=8080,
        help="Port to bind (default: 8080). Use 0 to auto-pick a free port.",
    )
    p_serve.add_argument(
        "--title",
        default="Specs dashboard",
    ...
    p_serve.add_argument(
        ...
        default=3600,
        metavar="SECONDS",
        help="Auto-shutdown after N seconds with no requests (default: 3600).",
    )
    p_serve.add_argument(
        "--no-idle-timeout",
        dest="idle_timeout",
        action="store_const",
        const=0,
        help="Disable the idle-timeout auto-shutdown (run until Ctrl-C).",
    )
    p_serve.add_argument(
        "--no-browser",
        action="store_true",
        help="Don't auto-open the browser after binding.",
    )
...
<div class="controls">
  ...
  <span class="chip" data-bucket="Planned">Planned<span class="n" id="n-Planned"></span></span>
...
<script>
  // SPECS/COUNTS/TOTAL start with embedded data (build mode) and get
  // overwritten by /api/specs response in serve mode. `let` so init()
  // can swap them out.
  let SPECS = JSON.parse(document.getElementById('specs-data').textContent);
  let COUNTS = __COUNTS_JSON__;
  let TOTAL = __TOTAL__;

  function applyMeta() {
    document.getElementById('n-all').textContent = ` ${TOTAL}`;
    for (const el of document.querySelectorAll('.chip .n')) {
      if (el.id !== 'n-all') el.textContent = '';
    }
  ...
  async function loadFromServer() {
    try {
      ...
      COUNTS = data.counts;
      TOTAL = data.total;
      const inputCode = document.querySelector('header .meta code');
      if (inputCode && data.input_dir) inputCode.textContent = data.input_dir;
      const gen = document.getElementById('meta-gen');
      if (gen && data.generated_at) gen.textContent = data.generated_at;
      return true;
    } catch (e) {
      return false;
    }
  ...
  function startHeartbeat() {
    // Keeps the server's idle-timeout watchdog from shutting down while a tab is open.
    setInterval(() => {
  ...
  async function init() {
    const live = await loadFromServer();
...
class IdleTimeoutServer(http.server.ThreadingHTTPServer):
    """HTTP server that auto-shuts-down after `idle_timeout` seconds with no requests.

    `idle_timeout=0` disables the watchdog (server runs until Ctrl-C).
    """

    def __init__(self, *args, idle_timeout: int = 3600, **kwargs):
    ...
    def _watchdog(self) -> None:
        # Wakes every 60s, exits when shutdown is requested or idle window elapses.
        while not self._shutdown_event.wait(60):
            if time.time() - self.last_request > self.idle_timeout:
                print(f"\nIdle for {self.idle_timeout}s, shutting down.")
...
class DashboardHandler(http.server.BaseHTTPRequestHandler):
    """Serves the dashboard HTML at /, live spec data at /api/specs, and /api/ping."""

    # Class-level config — set by cmd_serve before serve_forever.
    input_dir: Path = Path("docs/specs")
    excludes: set = set()
    legacy_aliases: bool = True
    title: str = "Specs dashboard"

    server: "IdleTimeoutServer"  # narrow the inherited type for last_request access
    ...
    def _serve_html(self) -> None:
        # Render with empty data — JS will fetch /api/specs on load.
        html = HTML_TEMPLATE
        html = html.replace("__TITLE__", self.title)
        html = html.replace("__DATA__", "[]")
        html = html.replace("__GENERATED_AT__", "live")
        html = html.replace("__TOTAL__", "0")
        html = html.replace("__COUNTS_JSON__", "{}")
        html = html.replace("__INPUT_DIR__", str(self.input_dir))
        self._send_bytes(200, "text/html; charset=utf-8", html.encode("utf-8"))

    def _serve_specs(self) -> None:
        specs = _load_specs(self.input_dir, self.excludes, self.legacy_aliases)
        counts: dict = {}
        for s in specs:
            counts[s["bucket"]] = counts.get(s["bucket"], 0) + 1
        payload = {
    ...
    def log_message(self, format: str, *args) -> None:
        # Silence the per-request access log; keep the console clean for the operator.
        # Errors still surface via log_error (which we don't override).
        pass
